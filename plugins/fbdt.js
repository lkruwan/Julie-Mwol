...
let teenuh = Config.WORKTYPE == 'public' ? true : false
...
//                               MAIN COMMAND
// -----------------------------------------------------------------------
Neotro.addCommand({ pattern: 'fb ?(.*)', fromMe: ramiya , desc: 'Facebook video download'}, async (message, match) => {
// අමාරුවෙන් යකො හැදුවෙ. උස්සන්න එපා තිරිසනෝ 😒
    const link = match[1]
...
Neotro.addCommand({ pattern: 'fb ?(.*)', fromMe: teenuh , desc: 'Facebook video download'}, async (message, match) => {
// අමාරුවෙන් යකො හැදුවෙ. උස්සන්න එපා තිරිසනෝ 😒
    const link = match[1]
